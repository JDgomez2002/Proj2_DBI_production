...
function Update_Incidence(){
    ...
    async function fetchPost1(){
        const { data } = await supabase.from('incidence').select().eq('patient_dpi',document.getElementById('input-dpi-paciente').value)
        setFile_ini(data)
    }
    async function fetchPost2(){

        const { data } = await supabase.from('incidence').select().eq('file_id',document.getElementById('input-state-file-id').value)
        setFile_fin(data)
    }
    async function fetchPost3(){
        console.log('disease id:', file_ini)
        console.log('disease fin:', file_fin)
        const { data } = await supabase.from('disease').select().eq('disease_id', file_fin[0].disease_id)
        setDisease_ini(data)
    }
    ...
    async function fetchPost5(){
        const { data } = await supabase.from('hospital').select().eq('hospital_id', file_fin[0].hospital_id)
    ...
    async function update_info() {
        ...
        await supabase.from('incidence').update({patient_dpi: patient_dpi, disease_id: disease_id, exam_id: exam_id, hospital_id: hospital_id, doctor_dpi: doctor_dpi, date_time: date_time, status: status, user_id: user_id}).eq('file_id',document.getElementById('input-state-file-id').value)
        .then(result => {
    ...
    return (
        <div className="update-incindence-mainpage-root" style={{display: "grid", placeItems: "center",}}>
            <div id="update-contact" className="container" style={{background: "rgba(0,0,0,0)"}}>
                <h3>Edición de expediente</h3>
                {statusComponent && statusComponent.split('\n').map((line, index) => (
                    <div key={index}>
                    {line}
                    <br />
                    </div>
                ...
                <fieldset>
                        <select id="input-dpi-paciente" className="cadaDetalle"  onChange={(e) => {setDpi(e.target.value); get_Info()}} >
                            <>
                            <option className="file" value="" >DPI del paciente: </option>
                                {lista_dpi&&(
                                <>
                                {lista_dpi.map(e=> (
                                    <>
                ...
                <fieldset>
                        <select id = "input-state-file-id" className="cadaDetalle"  onChange={(e) => {setFile(e.target.value), get_File()}} >
                            <>
                            <option className="file" defaultValue="" >Archivo que desea editar: </option>
                                {file_ini&&(
                                <>
                                {file_ini.map(e=> (
                                    <>
                ...
                <button className="buscar" onMouseOver={buscar} onClick={export_info} > Buscar</button>
                <fieldset>
                        <input id="fecha" className="cadaDetalle" type="date" />
                    </fieldset>
                    <fieldset>
                        <select id = "input-state-hereditary-diseases-id" className="cadaDetalle"  onChange={(e) => setDise_tot(e.target.value)} >
                            <>
                    ...
                    <fieldset>
                        <select id = "input-state-exam-id" className="cadaDetalle"  onChange={(e) => setExam_toto(e.target.value)} >
                            <>
                    ...
                    <fieldset>
                        <select id = "input-state-pais" name="state" className="form-control selectpicker" required value={hospital_tot} onChange={(e) => setHospital_tot(e.target.value)}>
                        {hospital&&(
                            <>
                    ...
                    <fieldset>
                        <select id = "doctores" name="doct" className="cadaDetalle" required value={doctor_tot} onChange={(e) => setDoctor_tot(e.target.value)}>
                        {doctor&&(
                            <>
                    ...
                    <button className="buscar" onClick={update_info} > Editar </button>
            </div>
        </div>
    )
}
