...
function Inventory() {
  ...
  async function fetchPostE() {
    ...
    const { data } = await supabase.from('inventory_medication').select()
    const medicamentosId = data.map((item) => item.medications_id)
  ...
  return (
    <div className="inventario-root1" style={{ display: "grid", placeItems: "center", margin: '100px 0px 0px 0px', width: '100%' }}>
      <div className="inventario1" style={{width: "300px"}} >
        <h1>Inventario</h1>

        <div className="button-container" style={{margin: "0px 0px 15px 0px"}}>
          <button
            className="boton"
            style={{width: "120px"}}
            onClick={() => {
